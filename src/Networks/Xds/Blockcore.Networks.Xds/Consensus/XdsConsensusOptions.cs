...
namespace Blockcore.Networks.Xds.Consensus
{
    ...
    public class XdsConsensusOptions : PosConsensusOptions
    {
        ...
        /// <inheritdoc />
        public override bool IsPosPowRatchetActiveAtHeight(int chainTipHeight)
        {
            if (this.currentNetwork.NetworkType == NetworkType.Testnet)
            {
        ...
        private void Assert(bool condition, [CallerMemberName] string caller = "", [CallerLineNumber] int line = -1)
        {
            if (!condition)
            {
                throw new Exception($"{nameof(XdsConsensusOptions)} - assert failed! Caller {caller}, line: {line}.");
            }
        }
    }
}